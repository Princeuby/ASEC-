...
class Committee_Model extends Officer_Model {
	...
	public function get_applicants_interview($vacancyID) {
		...
		if ($trainingStatus === null) {
			unset($conditions['training_status']);
			$conditions['training_status IS null'] = null;
		}
		$this->db->order_by('date_of_application');
		return $this->db->get_where('application', $conditions)->result_array();
	}
	...
	public function get_applicants_training($vacancyID) {
		$succesStatus = null;
		$conditions = array(
			'vacancy_id' => $vacancyID,
			'application_status' => '1',
		...
		if ($succesStatus === null) {
			unset($conditions['applicant_success']);
			$conditions['applicant_success IS null'] = null;
		}
		$this->db->order_by('date_of_application');
		return $this->db->get_where('application', $conditions)->result_array();
	}

	public function set_applicant_success($applicantID, $succesStatus) {
		if ($succesStatus === '1') {
			$data = array( // Data for update statement
			'applicant_success' => $succesStatus,
			'application_status' => '0'
